...
RMSNorm::RMSNorm(int dim, float eps) : dim_(dim), eps_(eps) {
  ...
  weight_ = ones({dim}, mlx::core::float32);
}
...
Linear::Linear(int in_features, int out_features, bool bias)
  ...
  weight_ = Tensor({out_features, in_features}, mlx::core::float32);

  // TODO: Initialize with random values instead of zeros
  // For now, use zeros (will be loaded from model weights)
  weight_ = zeros({out_features, in_features}, mlx::core::float32);

  if (has_bias_) {
    bias_ = zeros({out_features}, mlx::core::float32);
  }
}
...
Attention::Attention(int hidden_size, int num_heads, int max_seq_len)
    : hidden_size_(hidden_size),
      ...
      k_proj_(hidden_size, hidden_size, false),
      v_proj_(hidden_size, hidden_size, false),
...
Tensor Attention::forward(const Tensor& x, const Tensor* mask) {
  ...
  k = k.reshape({batch, seq_len, num_heads_, head_dim_});
  v = v.reshape({batch, seq_len, num_heads_, head_dim_});

  // Apply rotary embeddings
  auto [q_rot, k_rot] = rope_.forward(q, k);
  ...
  // k_rot transposed: [batch, num_heads, head_dim, seq_len]
  auto k_rot_t = k_rot.transpose({0, 1, 3, 2});
  auto scores = matmul(q_rot, k_rot_t);  // [batch, num_heads, seq_len, seq_len]
...
Tensor TransformerBlock::forward(const Tensor& x, const Tensor* mask) {
  ...
  auto attn_out = attention_.forward(normed, mask);
